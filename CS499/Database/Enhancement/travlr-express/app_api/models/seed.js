...
const dbURI = process.env.MONGODB_URI || `mongodb://root:root@${dbHost}:27017/travlr?authSource=admin`;
...
async function seedDatabase() {
    try {
        ...
        // Create users first (needed for news author refs)
        ...
        // Read and insert trips from JSON file (normalize fields)
        const dataFilePath = path.join(__dirname, '..', '..', 'data', 'trips.json');
        if (fs.existsSync(dataFilePath)) {
            const rawTrips = JSON.parse(fs.readFileSync(dataFilePath, 'utf8'));
            const now = new Date();
            const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
            const tripsData = rawTrips.map(t => {
                // Normalize length (e.g., "4 days" -> 4)
                let length = t.length;
                if (typeof length === 'string') {
                    const parsed = parseInt(length, 10);
                    if (!Number.isNaN(parsed)) length = parsed;
                }
                // Normalize perPerson price
                let perPerson = t.perPerson;
                if (typeof perPerson === 'string') {
                    const parsed = parseFloat(perPerson.replace(/[^0-9.]/g, ''));
                    if (!Number.isNaN(parsed)) perPerson = parsed;
                }
                // Normalize start date and ensure it is not in the past
                let start = new Date(t.start);
                if (!(start instanceof Date) || isNaN(start)) {
                    start = new Date(startOfToday.getTime());
                }
                if (start < startOfToday) {
                    // push to next month, same day or fallback to 1st
                    const day = Math.min(start.getDate() || 1, 28);
                    start = new Date(startOfToday.getFullYear(), startOfToday.getMonth() + 1, day);
                }
                return {
                    ...t,
                    code: String(t.code || '').toUpperCase().trim(),
                    length,
                    perPerson,
                    start
                };
            });
            await Trip.insertMany(tripsData, { ordered: true });
            console.log(`✓ Inserted ${tripsData.length} trips`);
        } else {
            console.log('⚠ trips.json not found, skipping trip data');
        }

        // Insert rooms (normalize pricePerNight)
        const normRooms = rooms.map(r => ({
            ...r,
            code: String(r.code || '').toUpperCase().trim(),
            pricePerNight: typeof r.pricePerNight === 'string' ? parseFloat(r.pricePerNight.replace(/[^0-9.]/g, '')) : r.pricePerNight
        }));
        await Room.insertMany(normRooms, { ordered: true });
        console.log(`✓ Inserted ${rooms.length} rooms`);

        // Insert meals (normalize price)
        const normMeals = meals.map(m => ({
            ...m,
            code: String(m.code || '').toUpperCase().trim(),
            price: typeof m.price === 'string' ? parseFloat(m.price.replace(/[^0-9.]/g, '')) : m.price
        }));
        await Meal.insertMany(normMeals, { ordered: true });
        console.log(`✓ Inserted ${meals.length} meals`);

        // Insert news (map author to admin user id, ensure publishDate is valid)
        const normNews = news.map(n => ({
            ...n,
            code: String(n.code || '').toUpperCase().trim(),
            author: adminUser._id,
            authorNameCached: 'Admin User',
            publishDate: n.publishDate ? new Date(n.publishDate) : new Date(),
        }));
        await News.insertMany(normNews, { ordered: true });
